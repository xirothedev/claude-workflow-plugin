/**
 * lifetime-classify — classify every pointer struct field into a Rust owner.
 *
 * Faithful TypeScript port of Bun PR #30412's `lifetime-classify.workflow.js`.
 *
 * Orchestration pattern: PIPELINE + SAMPLED 3-VOTE VERIFY.
 *   Classify   -> pipeline: one agent per .zig file classifies its *T fields
 *   Verify     -> select all UNKNOWN/low-confidence + a 12% random sample of
 *                 the confident ones, capped to stay under ~1000 agents at
 *                 scale; verify each with a 3-vote adversarial refute.
 *   Synthesize -> merge verdicts back, emit LIFETIMES.tsv + taxonomy stats.
 *
 * The Verify selection is *data-dependent* (it reads how many fields came back
 * UNKNOWN) and randomly sampled — only expressible on a live runtime.
 */
...
/** A classified field augmented with its source file + crate. */
interface FieldRow extends ClassifiedField {
  file: string;
  crate: string;
}

/** A field row after 3-vote verification. */
interface VerifiedRow extends FieldRow {
  refutes: number;
  final_class: string;
  verified: boolean;
}
...
JSC_BORROW   -> &JSGlobalObject / &VM etc.   — well-known JSC types always borrowed from caller; no struct lifetime needed
...
export async function execute(rt: WorkflowRuntime): Promise<unknown> {
  ...
  const FILES = (args.files as ZigFileArg[]) || [];
  if (FILES.length === 0) return { error: "no files" };

  phase("Classify");
  const classified = (await pipeline<ZigFileArg>(FILES, (_input, f) => {
    const sysHint = f.crate.endsWith("_sys")
      ? "this is a *_sys crate -> default FFI"
      : "this is NOT a *_sys crate";
    // (prompt condensed from original — full taxonomy + heuristics preserved.)
    return agent<ClassifyResult>(
...
Return ONLY fields that are pointer types. Skip [*]const u8 / []u8 (slices, handled separately).`,
    ...
    ).then((r): FieldRow[] => (r ? r.fields.map((x) => ({ ...x, file: f.zig, crate: f.crate })) : []));
  })) as FieldRow[][];

  const all: FieldRow[] = classified.flat();
  ...
  const verified = await parallel<VerifiedRow>(
    toVerify.map((f, i) => async () => {
      const votes = await parallel<Verdict>(
        Array.from(
          { length: 3 },
          (_unused, j) => () =>
...
Read the file. Check the evidence. Refute if: (a) the evidence line doesn't say what's claimed, (b) a different class fits better, (c) the rust_type is wrong for the class. If refuted, give correct_class.`,
      ...
      const consensus = refutes >= 2 ? votes.find((v) => v && v.refuted)?.correct_class ?? f.class : f.class;
      return { ...f, refutes, final_class: consensus, verified: true };
    }),
  );

  // merge verified back into all
  const vmap = new Map<string, VerifiedRow>(
    verified.map((v) => [`${v.file}|${v.struct}|${v.field}`, v]),
  );
  const final: VerifiedRow[] = all.map(
    (f) =>
      vmap.get(`${f.file}|${f.struct}|${f.field}`) || { ...f, final_class: f.class, verified: false, refutes: 0 },
  );
